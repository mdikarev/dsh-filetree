...
- Координация живого обновления (src/live-refresh.ts): чистые хелперы `parentDirectory`, `affectedExpandedDirectories`, trailing-edge debounce (250 мс) с объединением по пути, безопасный `parseSseChange` и `createLiveRefreshCoordinator` (один EventSource, reconnect с backoff, targeted invalidation, polling fallback).
...
- UI (клиент):
  ...
  - Panel (src/Panel.tsx): шторка-док 300px слева от контента (z-index 99, слот shell.overlay); шапка с именем корня (или «Файлы») и кнопками ↻ (обновить) и ✕ (закрыть); состояния: загрузка, ошибка («Повторить»), нет воркспейса, готово (дерево);
    владеет жизненным циклом координатора живого обновления (один EventSource, polling fallback со статус-баннером) и confirmation banner для изменённого на диске preview.
...
1. Открытая панель создаёт SSE-подписку `GET /filemanager-fs/events?hint=<workspace>&paths=<url-encoded JSON array>`. `paths` — относительные posix-пути раскрытых каталогов из store (корень workspace — пустая строка `""`); наблюдаются только эти каталоги, не весь workspace.
2. Сервер требует header `x-dsh-filemanager: 1`, разрешает hint (та же семантика, что у root/list/read: валидный каталог предпочтителен, иначе default root) и валидирует каждый путь (`realpath` + `isInside` + каталог, не `.git`) до создания любого watcher.
...
4. Координатор клиента держит ровно один EventSource, ключевой по hint и набору раскрытых путей; смена workspace или набора путей закрывает старое соединение до создания нового; события из устаревшего (старого workspace / перезапущенного) источника отбрасываются.
...
7. При ошибке EventSource клиент закрывает источник, переподключается с экспоненциальным backoff (база 500 мс, кап 10 с) и включает polling fallback: перечисление раскрытых каталогов раз в 5 секунд, сравнение снапшотов (имена/тип/размер/mtime) и инвалидация только изменившихся каталогов через тот же `refreshDirs`;
   успешный reconnect останавливает polling, панель показывает статус «Автообновление: опрос каталогов (SSE недоступен)».
8. При закрытии панели, смене workspace или остановке координатора закрываются EventSource, watcher-ы (через disconnect), debounce и polling-таймеры; дубликаты подписок не создаются, ручной ↻ продолжает работать.
...
Query:
- `hint`: строка (путь воркспейса; семантика как у root/list/read — валидный каталог предпочтителен, иначе default root)
- `paths`: URL-encoded JSON-массив относительных posix-путей раскрытых каталогов; корень workspace — пустая строка `""`; отсутствующий или пустой параметр означает «не наблюдать ничего»
...
- При disconnect/закрытии панели watcher-ы, EventSource и таймеры освобождаются (дубликаты подписок не возникают)
