...
7. При ошибке EventSource клиент закрывает источник, переподключается с экспоненциальным backoff (база 500 мс, кап 10 с) и включает polling fallback: перечисление раскрытых каталогов и корня раз в 5 секунд, сравнение снапшотов (имена/тип/размер/mtime) и инвалидация только изменившихся каталогов через тот же `refreshDirs`;
