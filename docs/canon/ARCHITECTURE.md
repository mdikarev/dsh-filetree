...
- UI (клиент):
  ...
  - image-вьюер (src/ImageView.tsx + чистый zoom-редуктор src/image-view.ts): fit + зум (кнопки/Ctrl+колесо, двойной клик 100%/fit, clamp 0.1–8×), размеры в тулбаре, «открыть оригинал» (raw-URL в новой вкладке), шахматная подложка через DSH-токены;
...
4. Вьюер: изображение вписывается по размеру панели; кнопки и Ctrl+колесо меняют масштаб (clamp 0.1–8×, шаг 1.25), двойной клик переключает 100%/fit; после загрузки показываются натуральные размеры; «открыть оригинал» открывает raw-URL в новой вкладке. Ошибка загрузки показывает сообщение с кнопкой повтора (смена `v`).
