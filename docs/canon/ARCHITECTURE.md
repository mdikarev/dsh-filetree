...
- UI (клиент):
  ...
  - Error-поверхность (src/notices.ts + src/ErrorToast.tsx): лёгкий стор уведомлений (`createNoticeStore` — push/dismiss/subscribe, dedupe по ключу, опциональный retry-action; autodismiss-таймером управляет компонент) и компонент тостов, рендерящийся в shell.overlay рядом с Panel.
    Ошибки листинга раскрытых каталогов и рефреша корня из `handleError` публикуются error-тостами с «Повторить» (повторный refresh затронутых каталогов); деградация SSE→polling из `coordinator.onError` — warning-тостом с авто-скрытием (баннер «опрос» остаётся длительным индикатором). Инлайн-ошибки корня, предпросмотра и диалога удаления не дублируются. Тосты локализованы, доступны (`role="alert"`).
